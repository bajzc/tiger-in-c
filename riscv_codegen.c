  ...
  switch (s->kind) {
    case T_MOVE: {
      ...
      if (dst->kind == T_MEM) {
        if (dst->u.MEM->kind == T_BINOP && dst->u.MEM->u.BINOP.op == T_plus &&
          ...
          emit(AS_Oper("lw `d0, 0(`s0)\n", L(t, NULL), L(munchExp(e2), NULL),
                       NULL));
          emit(AS_Oper("sw `d0, 0(`s1)\n", L(t, NULL), L(munchExp(e1), NULL),
                       NULL));
        } else {
          /* MOVE(MEM(e1),e2) */
    ...
    case T_JUMP: {
      if (s->u.JUMP.exp->kind == T_NAME) {
        ...
        /* JUMP(e) */
        emit(AS_Oper("j `s0\n", NULL, L(munchExp(s->u.JUMP.exp), NULL),
  ...
  switch (e->kind) {
    case T_BINOP: {
      ...
      if (e1->kind == T_CONST && e2->kind == T_CONST) {
        ...
        switch (op) {
          case T_plus: S("addi `d0, `s0, %d\n", constt); break;
          case T_minus: S("addi `d0, `s0, -%d\n", constt); break;
          default: assert(0);
        }
        ...
        /* BINOP(*,e1,e2) */
        switch (op) {
          case T_plus: S("add `d0, `s0, `s1\n"); break;
          case T_minus: S("sub `d0, `s0, `s1\n"); break;
          case T_mul: S("mul `d0, `s0, `s1\n"); break;
          case T_div: S("div `d0, `s0, `s1\n"); break;
          default: assert(0);
        }
