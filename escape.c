...
static void traverseVar(S_table env, int depth, A_var v) {
  ...
  case A_simpleVar: { // ID
    ...
    if (entry->depth < depth) {
      *(entry->escape) = TRUE;
      debug("In depth %d, set %s in depth %d to be escape\n", depth,
            S_name(v->u.simple), entry->depth);
    }
    break;
  }
  ...
  case A_functionDec: {
    for (A_fundecList l = d->u.function; l; l = l->tail) {
      ...
      for (A_fieldList L = f->params; L; L = L->tail) {
        L->head->escape = FALSE;
        debug("set function %s's argument %s to be non-escape\n",
