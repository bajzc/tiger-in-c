  ...
  case A_forExp: {
    ...
    beginBreakScope();
    {
      ...
      S_enter(venv, a->u.forr.var,
              E_VarEntry(lo.ty, Tr_allocLocal(level, TRUE)));
      ...
      level = Tr_newLevel(level, Temp_newlabel(), genEscapeList(funEntry));
      {
        ...
        for (A_fieldList L = f->params; L; L = L->tail, t = t->tail) {
          ...
          S_enter(venv, L->head->name,
                  E_VarEntry(t->head, Tr_allocLocal(level, TRUE)));
        }
  ...
  case A_varDec: {
    ...
    if (d->u.var.typ) { // vardec -> var id : type-id := exp
      ...
      if (ty->kind == Ty_record && e.ty->kind == Ty_nil) { // NULL
        S_enter(venv, d->u.var.var, E_VarEntry(ty, Tr_allocLocal(level, TRUE)));
        break;
    ...
    S_enter(venv, d->u.var.var, E_VarEntry(e.ty, Tr_allocLocal(level, TRUE)));
    break;
  }
  case A_typeDec: {
    A_nametyList l;
