...
static Ty_tyList makeFormalTyList(S_table tenv, A_fieldList params) {
  ...
  fprintf(stderr, "makeFormalTyList: ");
  TyList_print(l_head);
  fprintf(stderr, "\n");
  return l_head;
}
...
while (e && l) {
        struct expty eTy = transExp(venv, tenv, e->head);
      ...
      if (l)
        EM_error(a->pos, "passwd less arguments than needed");
    ...
    for (A_efieldList l = a->u.record.fields; l; l = l->tail) {
      ...
      if (fieldTy == NULL)
        EM_error(a->pos, "no member named '%s' in the record",
                 S_name(l->head->name));
      struct expty eTy = transExp(venv, tenv, l->head->exp);
  ...
  case A_seqExp: {
    ...
    for (l = a->u.seq; l; l = l->tail) {
      last_ty = transExp(venv, tenv, l->head);
    }
    return last_ty;
  }
  case A_assignExp: {
  ...
  case A_breakExp: // TODO check it's within for or while
		if(breakLevel<=0)
  ...
  case A_arrayExp: {
    ...
    fprintf(stderr, "arrayExp: init.ty: %p->%s\n", init.ty,
            str_ty[init.ty->kind]);
...
struct expty transVar(S_table venv, S_table tenv, A_var v) {
  ...
  case A_fieldVar: {
    struct expty var;
    var = transVar(venv, tenv, v->u.field.var);
    fprintf(stderr, "fieldVar: var.ty: %p\n", var.ty);
  ...
  case A_subscriptVar: {
    struct expty array = transVar(venv, tenv, v->u.subscript.var);
...
void transDec(S_table venv, S_table tenv, A_dec d) {
  ...
  case A_functionDec: {
    ...
    // Second, trans function body
    for (A_fundecList l = d->u.function; l; l = l->tail) {
      E_enventry funEntry = S_look(venv, l->head->name);
      A_fundec f = l->head;
      fprintf(stderr, "functionDec: translating %s\n", S_name(f->name));
      S_beginScope(venv, 1);
      {
        Ty_tyList t = funEntry->u.fun.formals;
        fprintf(stderr, "install function params\n\n");
        TyList_print(t);
        for (A_fieldList L = f->params; L; L = L->tail, t = t->tail) {
          fprintf(stderr, "functionDec: %s->%s\n", S_name(L->head->name),
                  str_ty[t->head->kind]);
          S_enter(venv, L->head->name, E_VarEntry(t->head));
        }
        fprintf(stderr, "\ninstall function params finished\n");
      }
      struct expty resultExp = transExp(venv, tenv, l->head->body);
  ...
  case A_varDec: {
    ...
    if (d->u.var.typ) { // vardec -> var id : type-id := exp
      ...
      fprintf(stderr, "varDec: %s : %s->type:%s\n", S_name(d->u.var.var),
              S_name(d->u.var.typ), str_ty[ty->kind]);
      fprintf(stderr, "varDec: ty: %p e.ty: %p\n", ty, e.ty);
